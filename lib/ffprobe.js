...
function parseFfprobeOutput(out) {
  ...
  // Parse a multiline XML tag
  var xmlLineIndex = lines.findIndex(function(line) {
      return line.indexOf('<?xml') !== -1;
  })
  if (xmlLineIndex !== -1) {
    while(!lines[xmlLineIndex + 1].match(/^TAG:/) && !lines[xmlLineIndex + 1].match(/^\[\//)) {
        lines[xmlLineIndex] += lines.splice(xmlLineIndex + 1, 1)[0].trim();
    }
  }
